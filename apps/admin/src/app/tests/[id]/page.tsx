    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            <Link href="/tests" className="text-emerald-600 hover:text-emerald-700 text-sm mb-2 inline-block">
              ← Back to Tests
            </Link>
            <p className="text-xs font-semibold text-emerald-600 uppercase tracking-wider mb-1">{test.test_number}</p>
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">{getTestTypeLabel(test.test_type)}</h1>
              <span className={`px-3 py-1 text-sm font-medium rounded-full ${getStatusColor(test.status)}`}>
                {test.status.replace('_', ' ')}
              </span>
            </div>
            {test.title && <p className="text-gray-500 text-sm mt-1">{test.title}</p>}
          </div>
