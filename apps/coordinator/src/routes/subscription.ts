...
export async function subscriptionRoutes(fastify: FastifyInstance) {
   ...
   * Registered in a scoped plugin so it gets its own content-type parser.
   */
  fastify.register(async (scope) => {
    // Override JSON parser to return raw Buffer (required for Stripe signature verification)
    scope.addContentTypeParser('application/json', { parseAs: 'buffer' }, (_req, body, done) => {
      done(null, body);
    });

    scope.post('/api/subscription/webhook', async (request, reply) => {
      ...
      try {
        switch (event.type) {
          case 'customer.subscription.updated': {
            ...
            if (!tenantId) break;
            const seats = sub.items?.data?.reduce((sum: number, item: any) => sum + (item.quantity || 1), 0) || 1;
            await pool.query(
              `UPDATE tenants
          ...
          case 'invoice.payment_succeeded': {
            const invoice = event.data.object;
            const customerId = invoice.customer as string;
            // Only update to active if currently past_due (trialing → active happens via subscription.updated)
            await pool.query(
              `UPDATE tenants
        ...
        // Still return 200 so Stripe doesn't retry for DB errors
      }
